...
if not df.empty and df.columns.any():
    ...
    remove_values = [
        ...
        "Бучацький р-н.,", "Бучацький р-н,", "47851, ", "Борщівський р-н, ", "Чортківський р-н,", "Гусятинський р-н,", "Тернопільська область, ", "СЬКА ОБЛ.,"
    ]
    ...
    ternopil['Факт.адресадоставки'] = ternopil['Факт.адресадоставки'].apply(remove_spaces)
    ternopil['Факт.адресадоставки'] = ternopil['Факт.адресадоставки'].apply(replacement_street).replace(",,",",")
    ...
    def plot_sales_by_region(data):
        ...
        for bar in bars:
            ...
            ax.text(xval / 2, yval, int(xval), ha='left', va='bottom', fontsize=6)
